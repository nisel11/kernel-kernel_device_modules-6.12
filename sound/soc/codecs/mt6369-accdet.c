...
static void mini_dump_register(void)
{
	...
	log_size +=
	snprintf(accdet_log_buf, sizeof(accdet_log_buf),
	...
	log_size += snprintf(accdet_log_buf + log_size, sizeof(accdet_log_buf) - log_size,
...
static void cat_register(char *buf)
{
	...
	ret = snprintf(accdet_log_buf, sizeof(accdet_log_buf), "[Accdet EINTx support][MODE_%d]regs:\n",
		accdet_dts.mic_mode);
	if (ret < 0)
	...
	ret = snprintf(accdet_log_buf, sizeof(accdet_log_buf), "[0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x\n",
		RG_INT_EN_ACCDET_ADDR,
		accdet_read(RG_INT_EN_ACCDET_ADDR),
		RG_INT_MASK_ACCDET_ADDR,
		accdet_read(RG_INT_MASK_ACCDET_ADDR),
...
static void mt6369_accdet_remove(struct platform_device *pdev)
{
	destroy_workqueue(accdet->eint_workqueue);
	destroy_workqueue(accdet->dis_micbias_workqueue);
	destroy_workqueue(accdet->accdet_workqueue);
...
static struct platform_driver mt6369_accdet_driver = {
	...
	.driver = {
		...
		.of_match_table = mt6369_accdet_of_match,
	},
};
