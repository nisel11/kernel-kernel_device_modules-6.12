			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	if (profile_name == NULL) {
		sized_strscpy(uinfo->value.enumerated.name, "NULL",
			strlen("NULL") + 1);
		return 0;
	}

	sized_strscpy(name, profile_name, sizeof(uinfo->value.enumerated.name));
			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	sized_strscpy(uinfo->value.enumerated.name,
