...
#define FTS_CSV_FILE_NAME                       "fts_testdata.csv"
#define FTS_TXT_FILE_NAME                       "fts_testresult.txt"
#define false 0
#define true  1
...
#define FTS_TEST_DBG(fmt, args...) do { \
	printk(KERN_WARNING"[FTS_TS][TEST]%s:"fmt"\n",  __func__, ##args); \
} while (0)

#define FTS_TEST_FUNC_ENTER() do { \
    printk(KERN_WARNING"[FTS_TS][TEST]%s: Enter\n", __func__); \
} while (0)

#define FTS_TEST_FUNC_EXIT()  do { \
    printk(KERN_WARNING"[FTS_TS][TEST]%s: Exit(%d)\n", __func__, __LINE__); \
} while (0)
