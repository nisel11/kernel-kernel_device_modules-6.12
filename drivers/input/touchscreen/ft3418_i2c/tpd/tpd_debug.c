...
#ifndef trace_MET_touch
#define trace_MET_touch(args...) do { } while (0)
#endif
...
void tpd_debug_set_time(void)
{
	...
	tpd_last_2_int_time[1] = (ts.tv_sec & 0xFFF) * 1000000 + (ts.tv_nsec / 1000);
			...
			int cal_x, int cal_y, int p, int down)
{
	...
	if ((tpd_down_status == 0 && down == 1) ||
		...
		trace_MET_touch("EV_KEY",
				ts.tv_sec, ts.tv_nsec / 1000, "BTN_TOUCH", down);
		tpd_down_status = !tpd_down_status;
	}
	if (tpd_down_status) {
		trace_MET_touch("EV_ABS", ts.tv_sec, ts.tv_nsec / 1000, "X", raw_x);
		trace_MET_touch("EV_ABS", ts.tv_sec, ts.tv_nsec / 1000, "Y", raw_y);
	}
}
