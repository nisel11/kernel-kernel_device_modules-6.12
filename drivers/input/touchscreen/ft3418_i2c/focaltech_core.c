...
#ifndef of_get_named_gpio_flags
#define of_get_named_gpio_flags(np, propname, index, flags) of_get_named_gpio(np, propname, index)
#endif
...
touchscreen_usb_plugin_data_t g_touchscreen_usb_pulgin;
...
static void fts_ts_usb_plugin_work_func(struct work_struct *work)
{
    ...
    if(lct_tp_get_dump_flag() || tp_get_self_flag()) {
        FTS_ERROR("lct_tp_get_dump_flag()=%d, tp_get_self_flag()=%d\n", lct_tp_get_dump_flag(), tp_get_self_flag());
        return;
    }
                                 ...
                                 unsigned long event, void *data)
{
    ...
    if (ts_data && data) {
        ...
        FTS_ERROR("ts_data or data is null");
        return -1;
    }
...
*/
    ...
    ret = mtk_disp_notifier_register("FocalTech Touch", &ts_data->fb_notif);
    if (ret) {
        FTS_ERROR("[DRM]Unable to register mtk_disp_notifier: %d\n", ret);
    }
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	...
	/* 2020.12.21 longcheer yangming add (xiaomi game mode ) end */
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (mtk_disp_notifier_unregister(&ts_data->fb_notif))
        FTS_ERROR("[DRM]Error occurred while unregistering mtk_disp_notifier.\n");
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
extern int tp_common_init(void);
extern int tp_selftest_init(void);
extern void tp_selftest_exit(void);
...
static int __init fts_ts_init(void)
{
    ...
    /* Unify sub-module initializations inside the single kernel module */
    tp_common_init();
    tp_selftest_init();
#ifdef TPD_DEBUG_CODE
    tpd_log_init();
