...
static ssize_t lct_proc_tp_palm_read(struct file *file, char __user *buf, size_t size, loff_t *ppos);
static ssize_t lct_proc_tp_palm_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos);
#include <linux/version.h>
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops lct_proc_tp_palm_fops = {
	.proc_read		= lct_proc_tp_palm_read,
	.proc_write		= lct_proc_tp_palm_write,
};
#else
...
static int lct_creat_proc_tp_entry(void)
{
	lct_tp_p->proc_entry_tp = proc_create_data(TP_PALM_NAME, 0444, NULL, &lct_proc_tp_palm_fops, NULL);
...
static ssize_t lct_proc_tp_palm_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	cnt = snprintf(page, PAGE_SIZE, "%s", (lct_tp_p->enable_tp_palm_flag ? "1\n" : "0\n"));
