...
#include "include/dsi-panel-mot-tm-td4376-672-fhd-dphy-vdo-120hz.h"
#endif
...
static long long screen_on_timestamp = 0;
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xB8, 0x31, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xB9, 0x7D, 0x5B, 0x00, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xBA, 0x7D, 0x5B, 0x00, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xCE, 0x5D, 0x60, 0x62, 0x65, 0x69, 0x6C, 0x76, 0x81, 0x8D, 0x99, 0xA4, 0xB0, 0xC2, 0xD4, 0xE8, 0xF3, 0xF7, 0xFF, 0x01, 0x7A, 0x0f, 0x0f, 0x00, 0x43, 0x69, 0x5a, 0x40, 0x43, 0x00, 0x00, 0x00, 0x64, 0xfa, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xC6, 0x02, 0x0A, 0x43, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x30, 0x13, 0x01, 0xAF, 0x0F, 0x01, 0x04, 0x1A, 0x1A, 0x1A, 0x00, 0x00, 0x00, 0x04, 0x05, 0x09, 0x00, 0x28, 0x01, 0x04, 0x1A, 0x1A, 0x1A, 0x01, 0x4B, 0x00, 0x00, 0x00, 0x1E, 0x01, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x20, 0x20, 0x00, 0x00, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x22, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1F, 0x08, 0xFC, 0xFF, 0xFF, 0xFF, 0x01, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xEF, 0x00, 0x40, 0x20, 0x03, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x17, 0x14, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x20, 0x03, 0xA0, 0x17, 0x14, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x03, 0x10, 0x03, 0x00, 0x10, 0x00, 0x10, 0x00, 0x0A, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4B, 0x00, 0x03, 0x51, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x05);
	...
	now = ktime_get_real();
	screen_on_timestamp = ktime_to_ms(now);
...
static void tm_panel_tp_reset(struct lcm *ctx)
{
	pr_info("%s:tm_td4376: +\n", __func__);
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	diff = timestamp - screen_on_timestamp;
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	tm_panel_tp_reset(ctx);

	if(kpi_log_level > 1) pr_info("disp: %s, lcd reset start\n", __func__);
	ctx->reset_gpio = devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->reset_gpio)) {
		dev_info(ctx->dev, "[error]%s: cannot get reset_gpio %ld\n", __func__, PTR_ERR(ctx->reset_gpio));
		return PTR_ERR(ctx->reset_gpio);
	...
	gpiod_set_value(ctx->reset_gpio, 1);
	usleep_range(20*1000,20*1000+1);
	devm_gpiod_put(ctx->dev, ctx->reset_gpio);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s+ disp:tm,td4376,672,vdo\n", __func__);
...
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
	FULL_PRODUCT_DEVICE_INFO(ID_LCD, "TD4376_FHDPLUS_DSI_VDO_TM");
#endif
	pr_info("[%d  %s]- tm_td4376 ret:%d  \n", __LINE__, __func__,ret);
...
static const struct of_device_id tm_of_match[] = {
	{
		...
		.compatible = "tm_td4376_vid_672_1080",
#else
		.compatible = "tm,td4376,672,vdo,120hz",
#endif
	},
	{}
};
...
static struct mipi_dsi_driver tm_driver = {
	.probe = lcm_probe,
	.remove = lcm_remove,
	.shutdown = lcm_shutdown,
	.driver = {
		.name = "tm_td4376_vid_672_1080",
		.owner = THIS_MODULE,
		.of_match_table = tm_of_match,
	},
};

module_mipi_dsi_driver(tm_driver);

MODULE_AUTHOR("mediatek");
MODULE_DESCRIPTION("tm td4376 incell 120hz Panel Driver");
