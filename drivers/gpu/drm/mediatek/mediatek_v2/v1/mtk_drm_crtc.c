...
/* For MT6858 */
#define MT6858_MMSYS_BYPASS_MUX_SHADOW 0xC00
#define MT6858_MMSYS_CROSSBAR_CON 0xC0C
...
void mtk_drm_crtc_mini_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6858:
		DDPDUMP("== DISP pipe0 MMSYS_CONFIG REGS:0x%pa ==\n",
			&mtk_crtc->config_regs_pa);
		mmsys_config_dump_reg_mt6858(mtk_crtc->config_regs);
		break;
...
void mtk_drm_crtc_mini_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6858:
		DDPDUMP("== DUMP DISP pipe-0 ANALYSIS:0x%pa ==\n",
			&mtk_crtc->config_regs_pa);
		mmsys_config_dump_analysis_mt6858(crtc);
		break;
	case MMSYS_MT6873:
	case MMSYS_MT6853:
	case MMSYS_MT6833:
...
void mtk_drm_crtc_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6835:
		...
		break;
	case MMSYS_MT6858:
		// mmsys_config_dump_analysis_mt6858(mtk_crtc->config_regs);
		mmsys_config_dump_analysis_mt6858(crtc);
		mutex_dump_analysis_mt6858(mtk_crtc->mutex[0]);
		mtk_vidle_dpc_analysis(); //test_541 need it?
		break;
	default:
		DDPPR_ERR("%s mtk drm not support mmsys id %d\n",
			__func__, priv->data->mmsys_id);
...
static void panel_feature_cmdq_cb(struct cmdq_cb_data data)
{
	struct mtk_cmdq_cb_data *cb_data = data.data;
	...
	unsigned int overlap_num, struct mtk_drm_lyeblob_ids *lyeblob_ids)
{
	...
	if (overlap_num == 0 && (bwm20_overlap != 0) &&
		...
		overlap_num = overlap_num - lyeblob_ids->overlap_ovl + bwm20_overlap;
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_OVL_BW_MONITOR) &&
			...
			tmp_overlap_of_bwm = lyeblob_ids->frame_weight_of_bwm -
				lyeblob_ids->overlap_ovl_of_bwm	+ bwm20_overlap;

			bw = bw_base * tmp_overlap_of_bwm / 400;
			DDPDBG("%s:%d BWM20 bw_base:%u overlap:%u bw:%u\n", __func__, __LINE__,
...
static void mtk_crtc_cmdq_timeout_cb(struct cmdq_cb_data data)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc) && (id == 0) &&
		...
		if ((display_compress_ratio_table[i].key_value) &&
			...
			if (!is_force_high_step &&
				...
				*(display_compress_ratio_table[i].peak_ratio));
				if (*(display_compress_ratio_table[i].average_ratio) == 0)
					*(display_compress_ratio_table[i].average_ratio) = 1000;
				if (*(display_compress_ratio_table[i].peak_ratio) == 0)
					*(display_compress_ratio_table[i].peak_ratio) = 1000;
			}
...
void mtk_crtc_start_bwm_ratio_loop(struct drm_crtc *crtc)
{
	...
	cmdq_pkt_flush_async(bwm_handle, bwm_trig_done_cb, (void *)mtk_crtc);
}
	...
	struct drm_atomic_state *state)
{
	...
	comp = priv->ddp_comp[DDP_COMPONENT_BWM0];
	if (!comp) {
		DDPMSG("%s %d\n", __func__, __LINE__);
		return;
	}
...
static void mtk_crtc_ddp_config(struct drm_crtc *crtc)
{
	...
	for (i = 0; i < mtk_crtc->layer_nr; i++) {
		...
		last_fence = *(unsigned int *)mtk_get_gce_backup_slot_va(mtk_crtc,
			DISP_SLOT_CUR_CONFIG_FENCE(mtk_get_plane_slot_idx(mtk_crtc, i)));
		cur_fence = (unsigned int)plane_state->pending.prop_val[
			PLANE_PROP_NEXT_BUFF_IDX];

		if (cur_fence != -1 && cur_fence > last_fence)
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				mtk_get_gce_backup_slot_pa(mtk_crtc,
					DISP_SLOT_CUR_CONFIG_FENCE(
						mtk_get_plane_slot_idx(mtk_crtc, i))),
				cur_fence, ~0);

		sub = 1;
		cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
			mtk_get_gce_backup_slot_pa(mtk_crtc,
				DISP_SLOT_SUBTRACTOR_WHEN_FREE(
					mtk_get_plane_slot_idx(mtk_crtc, i))),
			sub, ~0);
#endif
	...
	struct cmdq_pkt *handle)
{
	...
	if (panel_ext && panel_ext->dsc_params.enable) {
		...
		case MMSYS_MT6855:
			...
			break;
		case MMSYS_MT6858:
			mtk_ddp_remove_dsc_prim_mt6858(mtk_crtc, handle);
			break;
		default:
	...
	if (!doze_active ||
		...
		if (!mtk_crtc_is_ct_use_event(mtk_crtc))
			...
			if (cmdq_handle) {
				...
				cmdq_pkt_flush(cmdq_handle);
				cmdq_pkt_destroy(cmdq_handle);
				CRTC_MMP_EVENT_END(index, check_trigger, 0, 6);
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6983 ||
		...
		/*Set BYPASS_MUX_SHADOW*/
		writel(0x1, mtk_crtc->config_regs +
				MT6858_MMSYS_BYPASS_MUX_SHADOW);
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc, bool need_report_bw)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MAX_CHANNEL_HRT)) {
		mtk_crtc->usage_ovl_fmt[0] = 4;
		if (priv->data->mmsys_id == MMSYS_MT6989 && mtk_drm_dal_enable())
			mtk_crtc->usage_ovl_fmt[5] = 2;
...
void mml_cmdq_pkt_init(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	case MML_DIRECT_LINKING:
		if (mtk_vidle_is_ff_enabled() && mtk_crtc->is_mml) { // mml IR mode ddren low power
			cmdq_pkt_clear_event(cmdq_handle,
			       ...
			       struct mtk_plane_state *plane_state)
{
	...
	addr = mtk_get_gce_backup_slot_pa(mtk_crtc,
		DISP_SLOT_SUBTRACTOR_WHEN_FREE(
			mtk_get_plane_slot_idx(mtk_crtc, plane_index)));
