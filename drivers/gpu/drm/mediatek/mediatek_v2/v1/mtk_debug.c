...
static const struct proc_ops motUtil_proc_fops = {
	.proc_read = motUtil_read,
	.proc_write = motUtil_write,
	.proc_open = motUtil_open,
};
...
void disp_dbg_probe(void)
{
	...
	motUtil_procfs = proc_create("motUtil", S_IFREG | 0444,
				   ...
				   &motUtil_proc_fops);
	if (!motUtil_procfs) {
		DDPPR_ERR("[%s %d]failed to create motUtil in /proc/disp_ddp\n",
			__func__, __LINE__);
	}
