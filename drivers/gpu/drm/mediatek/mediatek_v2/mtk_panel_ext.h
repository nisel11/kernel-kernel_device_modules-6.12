...
#define READ_DDIC_SLOT_V2_NUM 66
...
#define MTK_MIPI_DSI_SKIP_CRTC_EN				BIT(5)
...
struct mtk_panel_funcs {
	...
	int (*lcm_update_roi_grp_cmdq)(void *dsi_drv,
		dcs_grp_write_gce cb, void *handle,
		unsigned int x, unsigned int y, unsigned int w, unsigned int h);
    ...
    int (*set_esd_recovery_flag)(int state);
};
