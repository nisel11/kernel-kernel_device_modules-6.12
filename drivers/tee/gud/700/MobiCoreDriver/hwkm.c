...
/* MMI_STOPSHIP <Security>: Keep the original logic for FBE enable, need rework later*/
/* FIXME
 * This implementation is still not thread-safe. The use of a global `km_session_counter`
 * and `g_km_session` without any locking mechanism creates a critical race condition.
 * Please protect all access to this shared session state with a mutex.
 */
