...
/*
 * MMU_shadow allocates a pair of shadow buffer and client virtual address
 */
struct mmu_shadow {
	void *buffer;
	void *cva;
...
struct tee_mmu {
	...
	u32				length;
	u32				flags;
	/* Position of the first page where the buffer address points to */
	u32				first_page_pos;
