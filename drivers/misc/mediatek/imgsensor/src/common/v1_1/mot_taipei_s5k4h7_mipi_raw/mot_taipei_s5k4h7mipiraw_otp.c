...
#include "mot_taipei_s5k4h7mipiraw_otp.h"
...
static OTP_INFO  otp_info_map = {
	...
	.af_sync_info.page_info.page[0] = 121,
	.af_sync_info.page_info.page[1] = 122,
	.af_sync_info.page_info.page[2] = 123,
	.af_sync_info.page_info.page_offset[0] = {0,0,0},
	.af_sync_info.page_info.page_offset[1] = {0,25,2},
	.af_sync_info.page_info.page_offset[2] = {0,13,14},
	.af_sync_info.page_info.page_addr[0] = 0x0A1F,
	.af_sync_info.page_info.page_addr[1] = 0x0A2B,
	.af_sync_info.page_info.page_addr[2] = 0x0A37,
	.af_sync_info.size = 27,//include flag

	.mtk_module_info.page_info.page[0] = 121,
	.mtk_module_info.page_info.page[1] = 123,
	.mtk_module_info.page_info.page[2] = 124,
	.mtk_module_info.page_info.page_offset[0] = {0,10,12},
	.mtk_module_info.page_info.page_offset[1] = {0,0,0},
	.mtk_module_info.page_info.page_offset[2] = {0,0,0},
	.mtk_module_info.page_info.page_addr[0] = 0x0A3A,
	.mtk_module_info.page_info.page_addr[1] = 0x0A06,
	.mtk_module_info.page_info.page_addr[2] = 0x0A12,
...
void *safe_memory_copy(char *dest, size_t dest_size, const char *src, size_t src_size)
{
	LOG_INF("dest_size is %ld, src_size is %ld", dest_size, src_size);
	if(dest == NULL || src == NULL){
		LOG_ERR("null pointer");
		return NULL;
	}
	if(0 == dest_size){
		LOG_ERR("the target size is 0");
		return NULL;
	}
...
int s5k4h7_otp_data(void)
{
	...
	COMMON_PAGE_INFO *af_sync_info        = &(otp_info_map.af_sync_info);
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.program_flag,
		(mot_s5k4h7_otp.light_source_flag - mot_s5k4h7_otp.program_flag)/sizeof(UINT8),
		mnf_info->data, mnf_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.wb_flag,
		(mot_s5k4h7_otp.oc_flag - mot_s5k4h7_otp.wb_flag)/sizeof(UINT8),
		awb_info->data, awb_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.af_inf_macro_flag,
		(mot_s5k4h7_otp.af_sync_flag - mot_s5k4h7_otp.af_inf_macro_flag)/sizeof(UINT8),
		af_info->data, af_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.lsc_flag,
		(mot_s5k4h7_otp.af_inf_macro_flag - mot_s5k4h7_otp.lsc_flag)/sizeof(UINT8),
		lsc_info->data, lsc_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.light_source_flag,
		(mot_s5k4h7_otp.wb_flag - mot_s5k4h7_otp.light_source_flag)/sizeof(UINT8),
		light_source_info->data, light_source_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.oc_flag,
		(mot_s5k4h7_otp.sfr_dist1_flag - mot_s5k4h7_otp.oc_flag)/sizeof(UINT8),
		optical_center_info->data, optical_center_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.sfr_dist1_flag,
		(mot_s5k4h7_otp.sfr_dist2_flag - mot_s5k4h7_otp.sfr_dist1_flag)/sizeof(UINT8),
		sfr_distance_1_info->data, sfr_distance_1_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.sfr_dist2_flag,
		(mot_s5k4h7_otp.sfr_dist3_flag - mot_s5k4h7_otp.sfr_dist2_flag)/sizeof(UINT8),
		sfr_distance_2_info->data, sfr_distance_2_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.sfr_dist3_flag,
		(mot_s5k4h7_otp.lsc_flag - mot_s5k4h7_otp.sfr_dist3_flag)/sizeof(UINT8),
		sfr_distance_3_info->data, sfr_distance_3_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.af_sync_flag,
		(mot_s5k4h7_otp.mtk_info_flag - mot_s5k4h7_otp.af_sync_flag)/sizeof(UINT8),
		af_sync_info->data, af_sync_info->size)){
		return -1;
	}
	...
	if(NULL == safe_memory_copy(mot_s5k4h7_otp.mtk_info_flag,
		(mot_s5k4h7_otp.mtk_info_crc + 2 - mot_s5k4h7_otp.mtk_info_flag)/sizeof(UINT8),
		mtk_module_info->data, mtk_module_info->size)){
		return -1;
	}
...
unsigned int mot_taipei_s5k4h7_read_region(struct i2c_client *client, unsigned int addr,
...
EXPORT_SYMBOL(mot_taipei_s5k4h7_read_region);
...
static void s5k4h7_check_af_data(mot_calibration_info_t * pOtpCalInfo)
{
	mot_s5k4h7_otp_t *pOTP  = &mot_s5k4h7_otp;

	pOtpCalInfo->af_status = STATUS_OK;

	if (!eeprom_util_check_crc16(pOTP->af_inf_macro_flag,
					(mot_s5k4h7_otp.af_crc - mot_s5k4h7_otp.af_inf_macro_flag)/sizeof(UINT8),
					read_uint16(pOTP->af_crc)))
	{
		LOG_ERR("AF inf & macro CRC Fails!");
		pOtpCalInfo->af_status = STATUS_CRC_FAIL;
		return;
	}
	else
		LOG_INF("AF inf & macro CRC Pass");

	if (!eeprom_util_check_crc16(pOTP->af_sync_flag,
					(mot_s5k4h7_otp.af_sync_crc - mot_s5k4h7_otp.af_sync_flag)/sizeof(UINT8),
					read_uint16(pOTP->af_sync_crc)))
	{
		LOG_ERR("Af sync data CRC Fails!");
		pOtpCalInfo->af_status = STATUS_CRC_FAIL;
		return;
	}
	else
...
int s5k4h7_get_cal_info(mot_calibration_info_t * pOtpCalInfo)
{
	...
	s5k4h7_check_af_data(pOtpCalInfo);
	s5k4h7_check_awb_data(pOtpCalInfo);
	s5k4h7_check_lsc_data(pOtpCalInfo);
	pOtpCalInfo->dual_status = STATUS_OK;

	LOG_INF("status mnf:%d, af:%d, awb:%d, lsc:%d, dual:%d", pOtpCalInfo->mnf_status, pOtpCalInfo->af_status, pOtpCalInfo->awb_status, pOtpCalInfo->lsc_status, pOtpCalInfo->dual_status);
	return 0;
}
