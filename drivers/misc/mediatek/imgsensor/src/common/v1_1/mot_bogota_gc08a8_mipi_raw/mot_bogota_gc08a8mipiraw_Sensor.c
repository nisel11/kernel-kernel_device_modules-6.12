 ...
 * Filename:
 * ---------
 *     mot_bogota_gc08a8_mipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "mot_bogota_gc08a8mipiraw_Sensor.h"
#include "mot_bogota_gc08a8_sensor_setting.h"

#define PFX "mot_bogota_gc08a8_camera_sensor"
...
#if MOT_BOGOTA_GC08A8_MIRROR_NORMAL
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,
#elif MOT_BOGOTA_GC08A8_MIRROR_H
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,
#elif MOT_BOGOTA_GC08A8_MIRROR_V
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,
#elif MOT_BOGOTA_GC08A8_MIRROR_HV
...
static void table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
	...
	while (len > idx) {
		...
		tosend = 0;
#endif
	//write_cmos_sensor_8bit(addr,data);
	LOG_INF("write 0x%x = 0x%x\n",addr, data);
	test_val = read_cmos_sensor(addr);
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
	case IMAGE_NORMAL:
                write_cmos_sensor_8bit(0x0101, 0x00);
		break;
	case IMAGE_H_MIRROR:
		write_cmos_sensor_8bit(0x0101, 0x01);
		break;
	case IMAGE_V_MIRROR:
		write_cmos_sensor_8bit(0x0101, 0x02);
		break;
	case IMAGE_HV_MIRROR:
                write_cmos_sensor_8bit(0x0101, 0x03);
		break;
	default:
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	stream_on_val = read_cmos_sensor(0x0100);
	pr_info("stream on X!stream_on_val = 0x%x \n",stream_on_val);
	return ERROR_NONE;
}
...
static void sensor_init(void)
{
	...
	table_write_cmos_sensor(mot_bogota_gc08a8_init_addr_data,
		sizeof(mot_bogota_gc08a8_init_addr_data)/sizeof(kal_uint16));
	LOG_INF("[%s] init_End\n", __func__);
}	/*	  sensor_init  */
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	table_write_cmos_sensor(mot_bogota_gc08a8_3264x1836_addr_data,
		sizeof(mot_bogota_gc08a8_3264x1836_addr_data)/sizeof(kal_uint16));
	LOG_INF("[%s] normal_video_End\n", __func__);
}

static void hs_video_setting(void)
{
	LOG_INF("[%s] hs_video_Start\n", __func__);
	table_write_cmos_sensor(mot_bogota_gc08a8_3264x2448_addr_data,
		sizeof(mot_bogota_gc08a8_3264x2448_addr_data)/sizeof(kal_uint16));
...
static int front_cam_get_info(char *buf, void *arg0)
{
    ...
    if (skuNumber > 0 && skuNumber < 10) {
        return sprintf(buf, "%s [%d*%d] %dM", "mot_bogota_gc08a8_front_dd_|_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("[mot_bogota_gc08a8_dd_v690_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("[mot_bogota_gc08a8_dd_v690_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 MOT_BOGOTA_GC08A8_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
