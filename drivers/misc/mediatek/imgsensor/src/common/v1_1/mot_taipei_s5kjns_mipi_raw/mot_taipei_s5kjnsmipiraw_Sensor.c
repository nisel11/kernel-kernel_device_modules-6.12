 ...
 * Filename:
 * ---------
 *	 mot_taipei_s5kjnsmipi_Sensor.c
 *
 * Project:
 * --------
 *	 MTK SOP
 ...
 *============================================================================
 ****************************************************************************/
#define PFX "MOT_TAIPEI_S5KJNS_camera_sensor"
...
#include "mot_taipei_s5kjnsmipiraw_Sensor.h"
#include "mot_taipei_s5kjns_Sensor_setting.h"
#include "mot_taipei_s5kjns_ana_gain_table.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_TAIPEI_S5KJNS_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	/* Preview*/
    { 8160, 6120,    0,    0, 8160, 6120, 4080, 3060,  0,   0, 4080, 3060,    0,    0, 4080, 3060}, // preveiw
	/* capture */
    { 8160, 6120,    0,    0, 8160, 6120, 4080, 3060,  0,   0, 4080, 3060,    0,    0, 4080, 3060}, // capture
	/* video */
    { 8160, 6120,    0,  764, 8160, 4592, 4080, 2296,  0,   0, 4080, 2296,    0,    0, 4080, 2296}, // video
	/* hs_video */
	...
	/* custom1 */
    { 8160, 6120,    0,  764, 8160, 4592, 2040, 1148,  0,   0, 2040, 1148,    0,    0, 2040, 1148}, // custom1
};
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[4] = {
	...
	{
		0x03, 0x0A, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2B, 0x7F8, 0x47C, 0x00, 0x00, 0x0000, 0x0000,
		0x01, 0x2B, 0x1FC, 0x23C, 0x00, 0x00, 0x0000, 0x0000
	},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4BlockNumX = 508,
	.i4BlockNumY = 382,
	.i4Crop = { {0, 0}, {0, 0}, {0, 382}, {0, 382}, {0, 0},
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_binning = {
	...
	.i4BlockNumX = 508,
	.i4BlockNumY = 380,
	.i4Crop = { {0, 0}, {0, 0}, {0, 382}, {0, 382}, {0,1},
...
#if MULTI_WRITE
kal_uint16 mot_taipei_s5kjns_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
kal_uint16 mot_taipei_s5kjns_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
	char puSendCmd[I2C_BUFFER_LEN];
...
static uint8_t taipei_s5kjns_read_actuator_id_from_eeprom(kal_uint8 slave, kal_uint16 eeprom_addr)
{
...
{
	uint8_t actuator_id = 0;
	actuator_id = taipei_s5kjns_read_actuator_id_from_eeprom(S5KJNS_EEPROM_SLAVE_ID, EEPROM_ACTUATOR_ID_POSITION);
	if(actuator_id == 0x30) {
...
int mot_taipei_s5kjns_bNeedSetNormalMode = 0;
/*************************************************************************
 * FUNCTION
 *	set_shutter
 *
...
static void set_shutter(kal_uint32 shutter)
{
	...
	if (shutter > 0xFFF0) {
		...
		mot_taipei_s5kjns_bNeedSetNormalMode = KAL_TRUE;
		if(shutter >= 4242424){
			shutter = 4242424;
		}
		...
		if (mot_taipei_s5kjns_bNeedSetNormalMode == KAL_TRUE) {
			mot_taipei_s5kjns_bNeedSetNormalMode = KAL_FALSE;
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < timeout; i++) {
			...
			if ( framecnt == 0xFF) {
				pr_debug("stream is not on, %d, i=%d", framecnt, i++);
				return ERROR_NONE;
			}
		}
...
{
	pr_debug("MOT TAIPEI S5KJNS init start\n");
	...
	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_init_mot_taipei_s5kjns,
		sizeof(addr_data_pair_init_mot_taipei_s5kjns)/sizeof(kal_uint16));

	if (xtc_data_valid == 1)
		write_xtc_data();

	pr_debug("MOT TAIPEI S5KJNS init end\n");

}	/*	  sensor_init  */

static void preview_setting(void)
{
	pr_debug("MOT TAIPEI S5KJNS preview_setting start\n");

	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_preview_mot_taipei_s5kjns,
		sizeof(addr_data_pair_preview_mot_taipei_s5kjns)/sizeof(kal_uint16));

	pr_debug("MOT TAIPEI S5KJNS preview_setting end\n");
...
{
	pr_debug("MOT TAIPEI S5KJNS capture_setting start\n");
	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_preview_mot_taipei_s5kjns,
		sizeof(addr_data_pair_preview_mot_taipei_s5kjns)/sizeof(kal_uint16));

	pr_debug("MOT TAIPEI S5KJNS capture_setting end\n");
}

static void normal_video_setting(kal_uint16 currefps)
{
	pr_debug("MOT TAIPEI S5KJNS normal_video_setting start\n");

	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_normal_video_mot_taipei_s5kjns,
		sizeof(addr_data_pair_normal_video_mot_taipei_s5kjns)/sizeof(kal_uint16));

	pr_debug("MOT TAIPEI S5KJNS normal_video_setting end\n");
}

static void hs_video_setting(void)
{
	pr_debug("MOT TAIPEI S5KJNS hs_video_setting start\n");

	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_120fps_s5kjns,
		sizeof(addr_data_pair_120fps_s5kjns)/sizeof(kal_uint16));

	pr_debug("MOT TAIPEI S5KJNS hs_video_setting end\n");
}

static void slim_video_setting(void)
{
	pr_debug("MOT TAIPEI S5KJNS slim_video_setting start\n");

	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_slim_video_mot_taipei_s5kjns,
		sizeof(addr_data_pair_slim_video_mot_taipei_s5kjns)/sizeof(kal_uint16));

	pr_debug("MOT TAIPEI S5KJNS slim_video_setting end\n");
}

static void custom1_setting(void)
{
	pr_debug("S5KJNS custom1_setting start\n");
	mot_taipei_s5kjns_table_write_cmos_sensor(addr_data_pair_60fps_s5kjns,
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->calibration_status, TAIPEI_S5KJNS_eeprom_get_calibration_status(), sizeof(mot_calibration_status_t));
	memcpy(&sensor_info->mnf_calibration, TAIPEI_S5KJNS_eeprom_get_mnf_info(), sizeof(mot_calibration_mnf_t));
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		mdelay(100); //add one max exposuretime to skip first frame
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 200250;//uint is ns
		break;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((*(feature_data + 0)) == 0) {
			*(feature_data + 0) =
				sizeof(mot_taipei_s5kjns_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)mot_taipei_s5kjns_ana_gain_table,
			sizeof(mot_taipei_s5kjns_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data) {
			...
			case MSDK_SCENARIO_ID_CUSTOM1:
				imgsensor_pd_info_binning.i4BlockNumX = 508;
				imgsensor_pd_info_binning.i4BlockNumY = 286;
				memcpy((void *)PDAFinfo, (void *)&imgsensor_pd_info_binning,
					sizeof(struct SET_PD_BLOCK_INFO_T));
				break;
			default:
...
UINT32 MOT_TAIPEI_S5KJNS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	return ERROR_NONE;
} /* MOT_TAIPEI_S5KJNS_MIPI_RAW_SensorInit */
