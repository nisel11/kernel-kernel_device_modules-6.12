 ...
 * Filename:
 * ---------
 *	 mot_bogota_sc820_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "mot_bogota_sc820_sensor_setting.h"

#define MOT_BOGOTA_SC820_SENSOR_GAIN_MAX_VALID_INDEX  6
...
//             kal_uint32 sensorID);
char mot_bogota_sc820_cameraSn[24] = {0};

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_BOGOTA_SC820_SENSOR_ID,
...
static kal_uint16 mot_bogota_sc820_table_write_cmos_sensor(
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < MOT_BOGOTA_SC820_SENSOR_BASE_GAIN)
		reg_gain = MOT_BOGOTA_SC820_SENSOR_BASE_GAIN;
	else if (reg_gain > MOT_BOGOTA_SC820_SENSOR_MAX_GAIN)
		reg_gain = MOT_BOGOTA_SC820_SENSOR_MAX_GAIN;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 MOT_BOGOTA_SC820_AGC_Param[MOT_BOGOTA_SC820_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x08 },
		{  4096,  0x09 },
		{  8192,  0x0B },
	...
	for (gain_index = MOT_BOGOTA_SC820_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= MOT_BOGOTA_SC820_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor8(0x3e08, MOT_BOGOTA_SC820_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * MOT_BOGOTA_SC820_SENSOR_BASE_GAIN / MOT_BOGOTA_SC820_AGC_Param[gain_index][0];
	write_cmos_sensor8(0x3e07, (temp_gain >> 3) & 0xff);
	LOG_INF( "MOT_BOGOTA_SC820_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d, gain = %d\n",
		MOT_BOGOTA_SC820_AGC_Param[gain_index][1], temp_gain, reg_gain, gain);
...
////min_frame_length
  ...
  if(target_frame_length > 1){
    ...
    write_cmos_sensor8(0x3e01, (shutter >> 4)&0xFF);
    write_cmos_sensor8(0x3e02, (shutter<<4) & 0xF0);
    LOG_INF( "SSSS Exit! shutter = %d, framelength = %d\n", shutter, imgsensor.frame_length);
...
{/* update20230213��wangruo */
	...
	write_cmos_sensor8(0x0100,0x00);
     ...
     mot_bogota_sc820_table_write_cmos_sensor(
         ...
         sizeof(addr_data_pair_init_mot_bogota_sc820) /
         sizeof(kal_uint16));
...
static int front_cam_get_info(char *buf, void *arg0)
{
	...
	return sprintf(buf, "%s [%d*%d] %dM", "mot_bogota_sc820_front_sw_|_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
}
#endif
...
UINT32 MOT_BOGOTA_SC820_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
